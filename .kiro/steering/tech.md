...
### デザインシステム
- **Apple Human Interface Guidelines準拠**: Clarity（明確さ）、Deference（控えめさ）、Depth（奥行き）の原則
- **包括的なデザインドキュメント**:
  - `docs/design-system.md`: 完全なデザインシステム仕様
  - `docs/design-rules.md`: 基本的なデザインルール
- **カラーパレット**:
  - プライマリ: Orange系統（orange-400/500/600）
  - セカンダリ: Amber系統（amber-300/400）
  - アクセント: Teal系統（teal-400/500）
  - ニュートラル: Stone系統（stone-50〜900）
- **8ptグリッドシステム**: 余白とレイアウトの一貫性
- **タイポグラフィスケール**: 9段階の階層（Display〜Caption）
- **コンポーネントライブラリ**: 再利用可能なUIパターン
- **アクセシビリティ重視**:
  - 最小タップターゲット: 44x44px（Apple推奨）
  - カラーコントラスト: WCAG AA基準準拠（4.5:1以上）
  - フォーカスリング: すべてのインタラクティブ要素に適用
